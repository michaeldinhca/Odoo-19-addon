...
class ExtendedWarrantyClaim(models.Model):
    _name = "extended.warranty.claim"
    _description = "Claim"
    _order = "priority,date desc"

    # id = fields.Integer('ID', readonly=True)
    name = fields.Char('Extended Claim Subject', required=True)
    ...
    def customer_details(self):
        ...
        warranty_claim_obj = self.env['product.warranty'].search(
            [('partner_id', '=', self.partner_id.id), ('is_extended_warranty','=', True),('state', 'not in', ['expired', 'new'])])
        prod = []
    ...
    def create(self, vals):
        ...
        return super(ExtendedWarrantyClaim, self).create(vals)

    @api.model
    def default_get(self, flds):
        result = super(ExtendedWarrantyClaim, self).default_get(flds)
