...
def migrate(cr, version):
    ...
    for integration in env['sale.integration'].search([]):
        ...
        if integration.is_integration_shopify:
            ...
            integration.field_ids.filtered(lambda x: x.name in ['version', 'graphql_version']).unlink()

            # 1.3 Migrate the receive_order_fulfillment_statuses
            ...
            # 1.4 Migrate the receive_order_financial_statuses
