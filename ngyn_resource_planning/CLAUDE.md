...
| Order tasks appear in within a project | Not our code — `loadData()`'s task `searchRead` passes no `order`, so it falls back to `project.task`'s own `_order` (`"priority desc, sequence, date_deadline asc, id desc"`). Stage is **not** part of this ordering even though it's now displayed (see below) — pass an explicit `order` in that `searchRead` call if stage- or charged-hours-based ordering is ever wanted here. |
