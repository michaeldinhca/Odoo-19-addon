...
| End-user documentation | `data/knowledge_articles.xml` — one root `knowledge.article`, `internal_permission: read` + an explicit `write` member for `base.partner_admin` (Knowledge has no group-based sharing, only per-person). Keep this in sync with real UI behavior when the screen changes — it's meant to be the actual end-user reference, not a stub. **The member is its own top-level `<record>`, not an inline `article_member_ids` eval** — a `(0, 0, {...})` create command on a one2many field re-creates a duplicate every time the data file reloads on upgrade, not just on first install (real production crash, fixed in `1.0.11`, see `CHANGELOG.md`). `internal_permission` is set to `'write'` in the article's first block and only dropped to `'read'` in a later block, after the member exists — `knowledge.article` requires an article to always have a writer, so doing it in the other order (or splitting the record without that ordering) throws on a fresh install. |
...
| Project lock (who can lock/unlock, what it blocks) | `models/project_project.py` — `x_ngyn_locked` + `action_ngyn_toggle_lock()` (PM-only, enforced in `write()` itself, not just the action). Enforcement call sites: `ngyn.task.assignment`/`ngyn.task.assignment.week`'s `create()`/`write()`/`unlink()` in `models/resource_assignment.py`, via `project.project._ngyn_check_plan_unlocked()`. JS: `canToggleLock()`/`toggleProjectLock()`/`_notifyLocked()` in `resource_planning_action.js`. Full behavior (what's blocked vs. deliberately exempt) documented in `docs/ARCHITECTURE.md` §"Project lock" — read that before changing what a lock does or doesn't cover. |
| Order tasks appear in within a project | `loadData()`'s task `searchRead` passes `order: "allocated_hours desc, id asc"` (most-charged-first, per request, `id` as a stable tiebreaker) — was previously unordered (falling back to `project.task`'s own `_order`). This sorts the one combined cross-project query; grouping it into each project's `pTasks` afterward via `.filter()` preserves that relative order, so it doesn't need to be re-sorted per project. Stage is **not** part of this ordering even though it's displayed (see below) — change the `order` string here if a different task sort is ever wanted. |
| Task stage / "+ Add team member" picker layout | Stage badge: `loadData()` fetches `stage_id`, rendered in `.o_task_stage` right after the task name in the template. Picker: `.o_picker_role_col` (480px) / `.o_picker_member_col` (search + `.o_picker_member_cloud` wrapping `.o_member_chip`s) in `resource_planning.scss` — role chips are always derived from `state.employees`, never a separate `hr.job` query, so an unused position can never appear there. |
| Over-charge warning (red styling + confirm-to-save dialog when planning more hours than a task is charged for) | `static/src/js/resource_planning_action.js` → `taskComputed()`'s `overCharged` flag, `onAllocChange()`/`onWeekChange()`'s shared `_confirmIfOverCharged()` gate. Styling: `.over` classes in `resource_planning.scss`. See `docs/ARCHITECTURE.md` § "Over-charge warning" for the exact trigger condition (only edits that newly create/worsen an overage prompt — reducing a number never does). |
