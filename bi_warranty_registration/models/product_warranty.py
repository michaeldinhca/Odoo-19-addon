...
class WarrantyDetails(models.Model):
    ...
    check_pro_warrnaty = fields.Boolean('Check Pro Warranty',compute="_check_pro_warrnaty")

    @api.depends('product_serial_id')
    def _check_pro_warrnaty(self):
        for rec in self:
            if rec.product_serial_id:
                find_lot = self.env['stock.lot'].sudo().search([('id','=',rec.product_serial_id.id)])
                if find_lot:
                    rec.product_serial_id.check_warranty = True
                    rec.check_pro_warrnaty = True
                else:
                    rec.check_pro_warrnaty = False
            else:
                rec.check_pro_warrnaty = False
    ...
    def action_view_invoice(self):
        action = self.env.ref('bi_warranty_registration.action_warranty_invoice_list1').read()[0]
        return action
