...
class WarrantySettings(models.Model):
	...
	create_warranty_from_purchaseorder = fields.Boolean('Create Warranty from Purchase Order')
	apply_on = fields.Selection([('product', 'Product'), ('variants', 'Product Variants'), ('category', 'Product Category')], default='category')
	warranty_from = fields.Selection([('sale', 'Sale Order'), ('purchase', 'Purchase Order')], default='sale')
	...
	def set_values(self):
		...
		self.env['ir.config_parameter'].sudo().set_param('bi_warranty_registration.create_warranty_from_purchaseorder', self.create_warranty_from_purchaseorder)
		module = False
		if self.create_warranty_from_saleorder == True or self.create_warranty_from_purchaseorder == True :
